	  	...
	  	<c:forEach var="select" items="${boardList}">	
			<thead>
				<tr class="trThead">
					...
					<th width="11%">학년</th>
					<th width="13%">금액</th>
					<th width="15%">입금 날짜</th>
					<th width="20%">결제 방법(카드, 계좌, 현금)</th>
					<th>메모</th>
				</tr>
			</thead>
			<tbody>
...
$(function() {
	...
	$(".trTbody").css("cursor", "pointer");	
	$(".trThead").css("pointer-events", "none");
	
// 	$(".trTbody").find("td").attr("style", "word-break:break-all");
	
	$(".trTbody").dblclick(function() { //dblclick
		var tr=$(this);
		var td=tr.children();
		var no = td.eq(0).text();
		//alert(no);
		/*$('#member_email').get(0).click(); */
 		window.open('modify?bno='+no+'','수정','width=500,height=670');
 		
		$(".trThead").click(function() {
			return false;
		});
	});
});
